...
# async def get_recommendations(cookies: Annotated[ProductCookies, 
# Cookie()]):
...
### The Curl Command:-curl -X POST -H "Cookie: session_id=abc123;
# preferred_category=Electronics; tracking_id=xyz789" http://127.0.0.1:8000/products/recommendations
...
## combining cookies and body parameters
...
class ProductCookies(BaseModel):
    ...
    title="Preferred Category", description="The user's preferred product category")

class PriceFilter(BaseModel):
    min_price: float = Field(ge=0.0, title="Minimum Price",
    description="The minimum price for product recommendations")

    max_price: float | None = Field(default=None, title="Maximum Price",
    description="The maximum price for product recommendations")
...
### The Curl Command:- curl -X POST -H "Cookie: session_id=abc123;preferred_category=Electronics" -H "Content-Type: application/json" -d "{\"price_filter\":{\"min_price\":50.0,\"max_price\":1000.0}}" http://127.0.0.1:8000/products/recommendations
